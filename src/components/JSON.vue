...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <!-- TODO: CODE TO RENDER LIST OF AUTHORS HERE -->
      <ul>
        <li v-for="author in modernAuthors" :key="author">
          {{ author.name }} ({{ author.birthYear }})
        </li>
      </ul>
      ...
      <ul>
        <li v-for="book in orwell.famousWorks" :key="book.title">
    ...
    <section class="lab-section">
      ...
      <p>Check if an author is modern:</p>
      <!-- Activity 14: Use v-if to conditionally render a message for modern authors. -->
      <ul>
        <li v-for="author in authors" :key="author.id">
          {{ author.name }}<span v-if="author.birthYear > 1850">: Modern Author</span> <span v-else>: Classic Author</span>
        </li>
      </ul>
    </section>
